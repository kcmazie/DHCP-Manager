...
$SendEmail = $true #False        #--[ Forces email to be sent ]--
#--[ If all are set to $false detected stats are displayed to screen and nothing else is done ]--
...
Function OptionUpdate ($ScopeName,$ScopeID,$Option,$OptionID,$OptionArray){   
    #--[ Options for Scope Update.  Tweak as needed ]------------------------
...
foreach ($Scope in $SiteScopes ){
    ...
    If ($RunUpdate){    #--[ Assorted scope adjustments.  !!! Comment these in or out as needed !!! ]--
        ...
        <#==[ Wireless Extras.  Disabled by default to avoid "accidents" ]===============
...
If ($SendEmail){
    ...
    If((get-date).DayOfWeek -eq "Sunday"){  #--[ Sends to main recipient only on Sunday so as not to spam ]--
        $Email.To.Add($ExtOption.Recipient) 
    }Else{
        $Email.To.Add($ExtOption.Recipient1)  
    #    $Email.To.Add($ExtOption.Recipient2)  #--[ add additional recipients ]--
    }
    If ($Over95 -gt 0){
...
<Settings>
	...
	<Email>
        ...
        <Recipient>helpdesk@mailbag.org</Recipient>
